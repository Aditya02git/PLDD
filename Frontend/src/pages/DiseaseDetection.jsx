...
const symptomsData = {
    ...
    Buffalo: ["Fever", "Cough", "Weight Loss", "Skin Lesions", "Blisters on gums", "Blisters on hooves", "Blisters on mouth", "Blisters on tongue", "Chest discomfort", "Chills", "Crackling sound", "Depression", "Difficulty walking", "Fatigue", "Lameness", "Loss of appetite", "Painless lumps", "Shortness of breath", "Sores on gums", "Sores on hooves", "Sores on mouth", "Sores on tongue", "Sweats", "Swelling in abdomen", "Swelling in extremities", "Swelling in limb", "Swelling in muscle", "Swelling in neck"],
    Goat: ["Fever", "Cough", "Weight Loss", "Skin Lesions", "Blisters on gums", "Blisters on hooves", "Blisters on mouth", "Blisters on tongue", "Chest discomfort", "Chills", "Crackling sound", "Depression", "Difficulty walking", "Fatigue", "Lameness", "Loss of appetite", "Painless lumps", "Shortness of breath", "Sores on gums", "Sores on hooves", "Sores on mouth", "Sores on tongue", "Sweats", "Swelling in abdomen", "Swelling in extremities", "Swelling in limb", "Swelling in muscle", "Swelling in neck"]
};
const DiseaseForm = () => {
    ...
    const fetchSolutions = async (disease, type) => {
        ...
        try {
            const API_KEY = "AIzaSyCXesk-awoAMHo9C4wJVp8dQewg3wVjPKY";
            const response = await axios.post(
                `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${API_KEY}`,
                {
                    contents: [{ parts: [{ text: `Provide homemade solutions for ${disease} in bullet points.` }] }]
                }
            );
            const text = response.data.candidates?.[0]?.content?.parts?.[0]?.text || "No solutions found.";
            const solutionsList = text.split("\n").filter(line => line.trim() !== "");
        ...
        type === "livestock" ? setFetchingLivestock(false) : setFetchingPlant(false);
    };
    ...
    const handleSymptomChange = (symptom) => {
        setSelectedSymptoms(prev =>
            prev.includes(symptom)
                ...
                : prev.length < 3 ? [...prev, symptom] : prev
        );
    };
    ...
    return (
        <>
            ...
            <div className=" p-10 text-black dark:text-white" style={{ textAlign: "center", marginTop: "100px" }}>
                <h2 className="text-[30px] md:text-4xl text-black dark:text-white font-bold mt-8">Livestock Disease Prediction</h2>
...
>
    ...
    <option value="Sheep" className="text-black dark:text-white">Sheep</option>
                    ...
                    {selectedAnimal && (
                        <>
                            <h3 className="text-black dark:text-white">Select Symptoms (Max 3)</h3>
                    ...
                    <div>
                        <button className="bg-green-700 hover:bg-green-800 m-4 p-4 rounded-md font-bold text-2xl text-center text-black dark:text-white " type="submit" disabled={loading}>{loading ? "Predicting..." : "Predict Livestock Disease"}</button>
                    </div>
                </form>
                {prediction && <p><strong>{prediction.disease}</strong> - {Math.round(prediction.confidence * 100)}%</p>}
                {prediction && (
                    <button className="bg-yellow-300 hover:bg-yellow-200 p-4 m-4 rounded-md font-bold text-2xl text-center text-black dark:text-white "onClick={() => fetchSolutions(prediction.disease, "livestock")} disabled={fetchingLivestock}>
                        {fetchingLivestock ? "Fetching..." : "Find Homemade Solution"}
                    </button>
                )}
    
                <div>
                    <h1 className="text-2xl font-semibold underline text-center text-black dark:text-white">Homemade Solutions for Livestock Disease</h1>
                    {fetchingLivestock ? <p>⏳ Loading solutions...</p> : <ul className="flex justify-between flex-wrap">{livestockSolutions.map((s, i) => <li key={i}>{s}</li>)}</ul>}
                </div>
    ...
    >
        ...
        {imagePreview ? (
            ...
            <p className="text-black dark:text-white">Drag & drop an image here, or click to select one</p>
                    ...
                    <button className="bg-green-700 hover:bg-green-800 p-4 m-4 rounded-md font-bold text-2xl text-center text-black dark:text-white " type="submit" disabled={loading}>{loading ? "Predicting..." : "Predict Plant Disease"}</button>
                </form>
                {plantPrediction && <p><strong>{plantPrediction.disease}</strong> - {Math.round(plantPrediction.confidence * 100)}%</p>}
                {plantPrediction && (
                    <button className="bg-yellow-300 hover:bg-yellow-200 p-4 m-4 rounded-md font-bold text-2xl text-center text-black dark:text-white " onClick={() => fetchSolutions(plantPrediction.disease, "plant")} disabled={fetchingPlant}>
                        {fetchingPlant ? "Fetching..." : "Find Homemade Solution"}
                    </button>
                )}
    
                <div>
                    <h1 className="text-2xl font-semibold underline text-center text-black dark:text-white">Homemade Solutions for Plant Disease</h1>
                    {fetchingPlant ? <p>⏳ Loading solutions...</p> : <ul className="flex justify-between flex-wrap">{plantSolutions.map((s, i) => <li key={i}>{s}</li>)}</ul>}
                </div>
            </div>
            <Footer />
        </>
